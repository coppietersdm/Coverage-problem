...
folder = "/home/matthieu/Documents/masterthesus/Coverage-problem/tests/nbr_drones"

# x = np.random.rand(60*2)*100
...
Ndrones = np.array([120,140,160,180])

N = 200
if(int(process) == 1):
    data = []
    endposes =[]
    for ndrones in Ndrones:
        print(ndrones)
        F = [ndrones]
        x1 = x.copy()[0:ndrones*2]
        for i in range(N):
            F.append(1 - fun(x1)/10000)
            x1 += max_step(algorithm1(x1), 0.2)
    ...
    np.savetxt(folder + "/gradient.txt", data)
    np.savetxt(folder + "/gradient_endpose.txt", endposes)
...
if(int(process) == 2):
    ...
    for ndrones in Ndrones:
        ...
        for i in range(N):
            F.append(1 - fun(x1)/10000)
            try:
                x1 += max_step(minimax(x1.reshape((-1,2)),polygon = default_polygon).reshape(-1), 0.2)
            except:
    ...
    np.savetxt(folder + "/minimax.txt", data)
    np.savetxt(folder + "/minimax_endpose.txt", endposes)
...
if(int(process) == 3):
    ...
    for ndrones in Ndrones:
        ...
        for i in range(N):
            F.append(1 - fun(x1)/10000)
            x1 += max_step(centroids(x1.reshape((-1,2)),polygon = default_polygon).reshape(-1), 0.2)
    ...
    np.savetxt(folder + "/centroids.txt", data)
    np.savetxt(folder + "/centroids_endpose.txt", endposes)
...
if(int(process) == 4):
    if(True):
        ...
        data = np.loadtxt("/home/matthieu/Documents/masterthesus/Coverage-problem/tests/nbr_drones/gradient.txt")
        for i in data:
            plt.plot(i[1:], label = str(int(i[0])) + ' drones')
        ...
        plt.title("Effect of the number of drones on the coverage ratio evolution")
        plt.ylabel("Coverage ratio")
        plt.xlabel("Iteration")
        plt.ylim(0.95,1.01)
        plt.legend()
        plt.savefig(folder + "/nbr_drones_evolution.pdf")
        plt.show()
    ...
    if(True):
        ...
        data = np.loadtxt(folder + "/gradient.txt")
        plt.plot(Ndrones, data.T[-1], 'o-', color = colors[0], label = 'gradient descent algorithm')
        cell_text.append(np.round(data.T[-1],3))
        rows.append("Gradient")
            
        data = np.loadtxt(folder + "/minimax.txt")
        plt.plot(Ndrones, data.T[-1],'o-', color = colors[1], label = 'minimax algorithm')
        cell_text.append(np.round(data.T[-1],3))
        rows.append("Minimax")
            
        data = np.loadtxt(folder + "/centroids.txt")
        plt.plot(Ndrones, data.T[-1],'o-', color = colors[2], label = 'centroids algorithm')
        cell_text.append(np.round(data.T[-1],3))
        rows.append("Centroids")
            
        plt.title("Effect of the number of drones on the coverage ratio")
        plt.ylabel("Coverage ratio")
        ...
        # plt.xticks(Ndrones)
        plt.legend()
        plt.savefig(folder + "/nbr_drones_coverage.pdf")
        plt.show()
    ...
    if(True):
        ...
        data = np.loadtxt(folder + "/gradient_endpose.txt")
        print(data)
        data -= x*(data != 0)
        data = np.sum(np.linalg.norm(np.array([t.reshape((-1,2)) for t in data]), axis = 2).T/Ndrones, axis=0)
        plt.plot(Ndrones, data, 'o-', color = colors[0], label = 'gradient descent algorithm')
        cell_text.append(np.round(data,3))
        rows.append("Gradient")
        
        data = np.loadtxt(folder + "/minimax_endpose.txt")
        data -= x*(data != 0)
        data = np.sum(np.linalg.norm(np.array([t.reshape((-1,2)) for t in data]), axis = 2).T/Ndrones, axis=0)
        plt.plot(Ndrones, data,'o-', color = colors[1], label ='minimax algorithm')
        cell_text.append(np.round(data,3))
        rows.append("Minimax")
        
        data = np.loadtxt(folder + "/centroids_endpose.txt")
        data -= x*(data != 0)
        data = np.sum(np.linalg.norm(np.array([t.reshape((-1,2)) for t in data]), axis = 2).T/Ndrones, axis=0)
        plt.plot(Ndrones, data,'o-', color = colors[2], label = 'centroids algorithm')
        cell_text.append(np.round(data,3))
        rows.append("Centroids")
        
        plt.title("Effect of the number of drones on the mean distance \n between the start and end position of the UAVs")
        ...
        # plt.xticks(Ndrones)
        plt.legend()
        plt.savefig(folder + "/nbr_drones_distances.pdf")
        plt.show()
    ...
    if(True):
        ...
        plt.title("Effect of the number of drones on the convergence time")
        ...
        # plt.xticks(Ndrones)
        plt.legend()
        plt.savefig(folder + "/nbr_drones_convergence.pdf")
        plt.show()
